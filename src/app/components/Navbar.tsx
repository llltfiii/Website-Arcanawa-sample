...
export function Navbar() {
  ...
  return (
    <motion.nav
      ...
      style={{
        backgroundImage: isScrolled ? 'none' : 'url(/src/app/Assets/navbar-background.jpg)', // Ganti dengan path gambar background yang diinginkan
          ...
          <Link to="/" className="relative group">
            ...
            >
              <div className="w-12 h-12 rounded-lg overflow-hidden">
                <ImageWithFallback
                  src="/src/app/Assets/Logo Arcanawa Interior Arsitek 2026.jpg"
                  alt="Arcanawa Logo"
                  className="w-full h-full object-cover"
                />
              </div>
              <div>
                <h1 className={`text-2xl font-bold tracking-tight ${isScrolled ? 'text-primary' : 'text-white'}`}>
                  ARCANAWA
                </h1>
                <p className={`text-xs tracking-widest ${isScrolled ? 'text-muted-foreground' : 'text-white/80'}`}>
                  ARCHITECTURE STUDIO
                </p>
              </div>
            </motion.div>
